...
import { CreditCardIcon, PlusIcon, TrashIcon } from '@heroicons/react/outline'
...
const PaymentMethods = () => {
    ...
    const handleSetPrimary = (methodId) => {
        ...
        // Ici, on mettrait à jour l'API pour définir la méthode comme principale
    }

    const handleDeleteMethod = (methodId) => {
        // Ici, on supprimerait la méthode de paiement via l'API
        ...
        <DashboardLayout title="Moyens de paiement">
            <div className="space-y-6">
                        ...
                        <RadioGroup value={selectedMethod} onChange={setSelectedMethod}>
                            <div className="space-y-3">
                                {paymentMethods.map(method => (
                                    <div
                                        key={method.id}
                                        className={`flex items-center justify-between p-4 border rounded-lg ${selectedMethod === method.id ? 'border-primary-500 ring-2 ring-primary-500' : 'border-gray-200'
                                            }`}
                                    >
                ...
                <div className="flex justify-end">
                    <Button
                        variant="primary"
                        onClick={() => handleSetPrimary(selectedMethod)}
                    >
